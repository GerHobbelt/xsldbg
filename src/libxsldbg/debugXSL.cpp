...
/***************************************************************************
                          debugXSL.cpp  - low level libxslt API handlers
                             -------------------
...
/*
 * Original file : debugXML.c : This is a set of routines used for
...
void addBreakPointNode(void *payload, void *data,
        xmlChar * name);


void addSourceNode(void *payload, void *data,
        xmlChar * name);


void addTemplateNode(void *payload, void *data,
        xmlChar * name);

void addGlobalNode(void *payload, void *data,
        xmlChar * name);

void addLocalNode(void *payload, void *data,
        xmlChar * name);


void addIncludeNode(void *payload, void *data,
        xmlChar * name);
         ...
         xmlChar * arg, xmlNodePtr source)
{
    ...
    if (arg[0] == 0) {
        ...
        if ((arg[0] == '-') && (xmlStrLen(arg) > 2)) {
            if (styleCtxt) {
                if (arg[1] == 't') {
                    ...
                    if (!templateNode) {
                        xsldbgGenericErrorFunc(QObject::tr("Error: The XSLT template named \"%1\" was not found.\n").arg(xsldbgText(&arg[offset])));
                        return result;
                    } else {
                        xsldbgGenericErrorFunc(QObject::tr(" template: \"%1\"\n").arg(xsldbgText(&arg[offset])));
                    ...
                    if (source) {
                        ...
                        xsldbgGenericErrorFunc(QObject::tr("Error: Unable to cd. No stylesheet loaded.\n"));
                    }
                } else {
                    xsldbgGenericErrorFunc(QObject::tr("Error: Unknown arguments to the command %1.\n").arg(QString("cd")));
                }
            } else
                xsldbgGenericErrorFunc(QObject::tr("Error: Unable to cd. No stylesheet loaded.\n"));
        } else {
            ...
            if (styleCtxt) {
                ...
                xsldbgGenericErrorFunc(QObject::tr("Error: Invalid arguments to the command %1.\n").arg(QString("cd")));
            }
        }
        ...
        if (list != NULL) {
                ...
                case XPATH_NODESET:
                    if (list->nodesetval) {
                        if (list->nodesetval->nodeNr == 1) {
                            ...
                            xsldbgGenericErrorFunc(QObject::tr("Warning: XPath %2 is a Node Set with %1 children.").arg(list->nodesetval->nodeNr).arg(xsldbgText(arg)) + QString("\n"));
                    } else {
                        xsldbgGenericErrorFunc(QObject::tr("Warning: XPath %1 is an empty Node Set.\n").arg(xsldbgText(arg)));
                    }
                    break;
            ...
            xsldbgGenericErrorFunc(QObject::tr("Error: XPath %1 was not found.\n").arg(xsldbgText(arg)));
        }
    ...
    if (arg == NULL) {
        ...
        modeTemp = NULL;
            ...
            if (callStackGetDepth() == 0)
                xsldbgGenericErrorFunc(QObject::tr("\tNo items in call stack.\n"));
            else
                xsldbgGenericErrorFunc("\n");
        }
    } else {
...
int xslDbgWalkContinue(void)
{
    ...
    /* We need to ensure that output is really sent. Otherwise
                 ...
                 void *data, VariableTypeEnum variableTypes)
{
    ...
    searchEmpty();
    xsldbgGenericErrorFunc(QObject::tr("Information: Updating search database. This may take a while...\n"));
    /* add items to the search dataBase */
    addCallStackItems();
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for breakpoints.\n"));
    walkBreakPoints((xmlHashScanner) addBreakPointNode, data);
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for imports and top-level stylesheets.\n"));
    walkStylesheets((xmlHashScanner) addSourceNode, data, style);
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for xsl:includes.\n"));
    walkIncludeInst((xmlHashScanner) addIncludeNode, data, style);
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for templates.\n"));
    walkTemplates((xmlHashScanner) addTemplateNode, data, style);
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for global variables.\n"));
    walkGlobals((xmlHashScanner) addGlobalNode, data, style);
    xsldbgGenericErrorFunc(QObject::tr("Information: Looking for local variables.\n"));
    walkLocals((xmlHashScanner) addLocalNode, data, style);
    xsldbgGenericErrorFunc(QObject::tr("Information: Formatting output.\n"));
...
void shellPrompt(xmlNodePtr source, xmlNodePtr doc, xmlChar * filename,
            ...
            xsltTransformContextPtr styleCtxt)
{
    ...
    if (nextCommandActive)
      nextCommandActive = 0;
        ...
        commandId = lookupName((const char*)command, commandNames);
        /* try command shorts if command is not found */
        if (commandId < 0)
            commandId =
                lookupName((const char*)command, shortCommandNames);
        ...
        xsldbgValidateBreakpoints = BREAKPOINTS_NEED_VALIDATION;
          ...
          cmdResult = xslDbgShellOutput(arg);
            ...
            case DEBUG_CD_CMD:
                ...
                dir[2] = '\0';
                shortCutId = lookupName((const char*)dir, cdShortCuts);
                if (shortCutId >= 0) {
                    if (xmlStrLen(arg) == 2) {
       ...
       case DEBUG_SHOWWATCH_CMD:
         ...
         default:
           ...
           break;
            ...
            case DEBUG_LOAD_CMD:
            case DEBUG_READCONFIG_CMD:
                if (xsldbgReadConfig(arg)){
                    cmdResult = 1;
                    xslDebugStatus = DEBUG_RUN_RESTART;
                }
                break;
