...
#include <libxsldbg/xsldbgconfig.h>
#include <windows.h> // needed for thread creation
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>
...
FILE *stdoutIO = NULL;
long WINAPI xsldbgThreadMainWin32(long lParam);
...
xsldbgThreadInit(void)
{
    ...
    /* create the thread */
	hThread = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)xsldbgThreadMainWin32,NULL,0,&iID);
	if (!hThread){
	  CloseHandle (hThread);
      ...
      for (counter = 0; counter < 11; counter++){
		if (getThreadStatus() != XSLDBG_MSG_THREAD_INIT)
			break;
		Sleep(250); /*guess that it will take at most 2.5 seconds to startup */
      }
...
xsldbgThreadFree(void)
{
  fprintf(stderr, "xsldbgThreadFree()\n");
	...
	if (getThreadStatus() == XSLDBG_MSG_THREAD_DEAD)
	  break;
	Sleep(250); /*guess that it will take at most 2.5 seconds to stop */
      }
    }
...
long 
WINAPI xsldbgThreadMainWin32(long lParam)
{
  int defaultArgc = 2;
  char *defaultArgv[2];
  int i;
